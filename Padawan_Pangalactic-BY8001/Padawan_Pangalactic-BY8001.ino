...
//       r0n_dL Edit to include BY8001-16P Support                               //
//            - Had issues with selecting PS2 and commented out usb writing to   //
//              screen to resolve...until I can figure it out                    //
...
#define CONTROL 0 // 0=PS2 (wireless PS2 Receiver connected to pins )
...
SoftwareSerial MP3Serial2(8, 6);  // SoftwareSerial port assignments (RX, TX) to communicate with sound module 2
  ...
  BY8x0116Pv2 priPlayer(MP3Serial1);
#else 
  ...
  #include <BY8x0116Pv2.h>
  #endif
  BY8x0116Pv2 secPlayer(MP3Serial2);
#else 
  //settings for Sparkfun MP3 Trigger...
  #include <MP3Trigger.h>
...
void playSound(int soundNumber) {
    ...
    for (byte i = 0; i < sizeof(muzak); i++) {
      if (soundNumber==muzak[i]) {
        secPlayer.playFile(soundNumber); //play file on secondary player
        sec=1;
      }
    }
    if (sec==0) priPlayer.playFile(soundNumber); //play file on primary player
  #else
    priPlayer.playFile(soundNumber); //only one audio player is configured. very sad.
  #endif
}
//end of playSound()
...
void setVolume(byte vol) {
  ...
  #if (AUDIO2>0)
    #if (AUDIO2==2)
      secPlayer.setVolume(vol); //set BY8001 volume
    #else
